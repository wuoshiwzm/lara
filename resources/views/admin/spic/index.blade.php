			...
			<div class="result_wrap">
					<div class="result_content">
							<table class="list_tab">
									...
									<tr>
											<td class="tc">
													<input type="text" onchange="changeOrder(this,{{$v->spic_id}})" value="{{$v->spic_order}}">
											</td>
...
function changeOrder(obj,spic_id){
	...
	$.post("{{url('admin/spic/changeorder')}}",{'_token':'{{csrf_token()}}','spic_id':spic_id,'spic_order':spic_order},function(data){
...
function delSpic(spic_id){
	...
	layer.confirm('是否删除分类？', {
	  btn: ['确认','取消']
			...
			$.post("{{url('admin/spic/')}}/"+spic_id,{'_method':'delete','_token':"{{csrf_token()}}"},function(data){
				if(data.status==0){
